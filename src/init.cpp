...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Icochain.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Icochaind.pid)") + "\n";
    ...
    strUsage += "  -maxconnections=<n>    " + _("Maintain at most <n> connections to peers (default: 10)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Icochain is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Icochain is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Icochain version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Icochain server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Icochain") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Icochain to complete") << "\n";
    ...
    if (!GetBoolArg("-staking", true))
        LogPrintf("Staking(POS) Disabled\n");
    else if (pwalletMain)
    ...
    if(miningThreads >0)
    {
        ...
        if(nProcessors > miningThreads) nProcessors = miningThreads;
        for(int i = 0;i<nProcessors;i++)
            threadGroup.create_thread(boost::bind(&ThreadWorkMiner, pwalletMain));
    }
    else
    {
        LogPrintf("Mining(POW) Disabled\n");
    }
