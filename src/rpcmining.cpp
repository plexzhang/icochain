...
extern volatile bool fEnableMining;
extern volatile bool fEnableStaking;
...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetPowNewIssue(pindexBest->nPowHeight+1);
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetPosNewIssue(pindexBest->nHeight - pindexBest->nPowHeight);
}

Value setmining(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "setmining <mining>\n"
            "<mining> is true or false to turn mining(POW) on or off.\n");


    int miningThreads = GetArg("-miningthreads", 2);
    if(miningThreads <= 0)
    {
        throw runtime_error(
            "Invalid instruction\n"
            "Please enable mining and set the mining thread(s) in the configuration file before start the node.\n"
            "usage: -miningthreads=<number of threads>\n");
    }

    fEnableMining = params[0].get_bool();

    Object obj;
    if(fEnableMining)
    {
        obj.push_back(Pair("message", "Enable Mining(POW) Success!"));
        obj.push_back(Pair("miningthreads", miningThreads));
    }
    else
        obj.push_back(Pair("message", "Disable Mining(POW) Success!"));
...
Value setstaking(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "setstaking <minting>\n"
            "<minting> is true or false to turn POS generation on or off.");

    if (!GetBoolArg("-staking", true))
    {
        throw runtime_error(
            "Invalid instruction\n"
            "Please enable staking in the configuration file before start the node.\n"
            "usage: -staking=1\n");
    }

    fEnableStaking = params[0].get_bool();

    Object obj;
    if(fEnableStaking)
        obj.push_back(Pair("message", "Enable Staking(POS) Success!"));
    else
        obj.push_back(Pair("message", "Disable Staking(POS) Success!"));
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    Object obj, diff, weight,pow,pos;
    ...
    pow.push_back(Pair("startmining",   fEnableMining));
    pow.push_back(Pair("mining",        fEnableMining ? GetMiningStatus() : false));
    pow.push_back(Pair("miningthreads", fEnableMining && GetMiningStatus() ? GetArg("-miningthreads", 2) : 0));
    pow.push_back(Pair("myhashps",       gethashespersec(params, false)));
    obj.push_back(Pair("netmhashps",     GetPoWMHashPS()));
    obj.push_back(Pair("pow",    pow));

    pos.push_back(Pair("startstaking", fEnableStaking));
    pos.push_back(Pair("staking",       fEnableStaking ? GetStakingStatus() : false));
    obj.push_back(Pair("netstakeweight", GetPoSKernelPS()));
    obj.push_back(Pair("pos",    pos));

    obj.push_back(Pair("pownewissue",    (uint64_t)GetPowNewIssue(pindexBest->nHeight+1)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Icochain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Icochain is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Icochain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Icochain is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx)
    {
        ...
        {
            entry.push_back(Pair("fee", (int64_t)(tx.GetValueIn(0,mapInputs) - tx.GetValueOut(0))));
